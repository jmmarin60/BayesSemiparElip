...
#
# Bayesian inference for a (multivariate) normal distribution: 
# x[i] = mu + A Z[i]/sqrt(w)
...
# positive definite matrix such that all of the diagonal elements sum to 1 and w
...
#
# Note that cw and dw are integrated out.
#
rnormal <- function(n, mu, Sigma){
  ...
  if (is.vector(Sigma)){Sigma <- Sigma*diag(1)}
  x <- LaplacesDemon::rmvn(n, mu, Sigma)
  return(x)
}
#
likelihood.norm <- function(x, mu, Sigma, log = TRUE){
  #
  # Calculates the (log-)likelihood function for a sample of (multivariate)
  # normal data and a set of parameter values.
  #
  if (!require("LaplacesDemon")){install.packages("LaplacesDemon")}
  d <- length(mu)
  if (d == 1){
    ...
    f <- sum(dnorm(x, mu, sqrt(Sigma), log = TRUE))
  }else{
    f <- sum(LaplacesDemon::dmvn(x, mu, Sigma, log = TRUE))
  }
  
  return(ifelse(log == TRUE, f, exp(f)))
}
#
priors.norm <- function(d){
  #
  # Fixes priors.
  #
  priors <- list()
...
inits.norm <- function(x){
  ...
  inits$aw <- 1
  inits$bw <- 1
  inits$Sigma <- inits$Sigma*inits$w
  if (d > 1){
...
#
generate.w.norm <- function(x, aw, bw, mu, Sigma){
  #
  ...
  n <- dim(x)[1]
  aw <- aw+0.5*n*d
  bw <- bw+sum(mahalanobis(x, mu, Sigma))/2
  return(rgamma(1, aw, bw))
}
#
generate.awbw.norm <- function(w, oldaw, oldbw, acw, bcw, adw, bdw, 
                               par = rep(0.025, 2)){
  #
  # Generates from the conditional posterior distribution of aw, bw | w.
  #
  acc <- 1
  aw <- rlnorm(1, log(oldaw), par[1])
  bw <- rlnorm(1, log(oldbw), par[2])
  paccept <- log(aw)+log(bw)-log(oldaw)-log(oldbw)+
    dgamma(w, aw, bw, log = TRUE)-dgamma(w, oldaw, oldbw, log = TRUE)+
    LaplacesDemon::dpareto(bcw+aw, acw, log = TRUE)-
    LaplacesDemon::dpareto(bcw+oldaw, acw, log = TRUE)+
    LaplacesDemon::dpareto(bdw+bw, adw, log = TRUE)-
    LaplacesDemon::dpareto(bdw+oldbw, adw, log = TRUE)
...
#
generate.mu.norm <- function(n, sumx, invSigma, w, m, invV){
  #
  # Generates from the conditional posterior distribution of mu | x, Sigma, w.
  #
  if (!require("LaplacesDemon")){install.packages("LaplacesDemon")}
  if (is.vector(invV)){invV <- matrix(invV, ncol = 1)}
  V <- solve(invV+n*w*invSigma)
  V <- make.symmetric(V)
  m <- as.vector(V%*%(invV%*%m+w*invSigma%*%sumx))
  mu <- LaplacesDemon::rmvn(1, m, V)
  return(as.vector(mu))
}
#
generate.sigma2.norm <- function(x, w, oldSigma, R, oldloglik, asigma2, 
                                 par = 0.025){
  #
  ...
  Sigma <- Sigma%*%R%*%Sigma
  loglik <- likelihood.norm(x, mu, Sigma/w)
...
#
generate.R.norm <- function(x, w, oldSigma, oldR, oldD, oldW, oldloglik, 
                            par = 500){
  #
...
for (jindex in 1:numrep){
  ...
  #   n <- 1000
  #   true.mu <- c(-3, 3, 10) # c(-3,5) # -3 # 
  #   true.Sigma <- matrix(c(3, 1, 1, 1, 2, 1, 1, 1, 4), nrow = 3) # matrix(c(3,1,1,3), ncol = 2) # 5 # 
  #   if (is.vector(true.Sigma)){true.Sigma <- matrix(true.Sigma, ncol = 1)}
  #   true.w <- sum(diag(true.Sigma))
  #   true.scaled.Sigma <- true.Sigma/true.w
  #   x <- rnormal(n, true.mu, true.Sigma)
  # }else{
  ...
  #
  sumx <- summary.stats.norm(x)$sumx
  #
  # Set priors.
  #
  priors <- priors.norm(d)
  acw <- priors$acw
  bcw <- priors$bcw
  adw <- priors$adw
  bdw <- priors$bdw
  ...
  if (plots == TRUE){
    ...
    if (simulate.data == TRUE){
      ...
      for (j in 1:d){
        true.marg.f[ , j] <- dnorm(grid[, j], true.mu[j], sqrt(true.Sigma[j, j]))
      }
      if (d > 1){
  ...
  if (dic == TRUE){
    ...
    fhat <- rep(0, n)
    # if (simulate.data == TRUE){
    #   true.dev <- -2*likelihood.norm(x, true.mu, true.Sigma, log = TRUE)
    # }
  }
  #
  # Start sampler.
  ...
  for (i in 1:totits){
    w <- generate.w.norm(x, aw, bw, mu, Sigma)
    result <- generate.awbw.norm(w, aw, bw, acw, bcw, adw, bdw, par.awbw)
    awbw.acc <- awbw.acc+result$acc
    aw <- result$aw
    bw <- result$bw
    mu <- generate.mu.norm(n, sumx, invSigma, w, m, invV)
    if (d > 1){
      loglik <- likelihood.norm(x, mu, Sigma/w)
      result <- generate.sigma2.norm(x, w, Sigma, R, loglik, asigma2, par.sigma2)
      Sigma <- result$Sigma
      loglik <- result$loglik
      sigma2.acc <- sigma2.acc+result$acc
      result <- generate.R.norm(x, w, Sigma, R, D, W, loglik, par.R)
      Sigma <- result$Sigma
      invSigma <- solve(Sigma)
      R <- result$R
      D <- result$D
    ...
    if (i > burnin){
      ...
      if (plots == TRUE){
        for (j in 1:d){
          marg.f[ , j] <- marg.f[ , j] + dnorm(grid[ , j], mu[j], 
                                               sqrt(Sigma[j, j]/w))
        }
        if (d > 1){
          for (j in 1:d.minus.1){
            for (k in (j+1):d){
              bivar.f[ , , j, k] <- bivar.f[ , , j, k]+
                LaplacesDemon::dmvn(cbind(rep(grid[,j], npts.plus.1),
                                          rep(grid[,k], each = npts.plus.1)), c(mu[j], mu[k]), 
                                    matrix(c(Sigma[j, j], Sigma[j, k], Sigma[j, k], Sigma[k, k]), 
                                           nrow = 2)/w)
            }
          }
        }
      ...
      if (dic == TRUE){
        dev <- dev+loglik
        if (d == 1){
          fhat <- fhat+dnorm(x, mu, sqrt(Sigma/w))
        }else{
          fhat <- fhat+LaplacesDemon::dmvn(x, mu, Sigma/w)
        }
      }
      # Save posteriors
